...
from .serializers import ArticleFindSerializer, ArticleFindListSerializer, ArticleFindDetailSerializer, CommentFindSerializer, CommentFindListSerializer, ArticleFindSearchSerializer
from .models import Article, Comment
from articles_lost.models import Animal, Species, City, Borough
from rest_framework.permissions import IsAuthenticated
from django.http import HttpResponse, HttpResponseForbidden
from drf_yasg.utils import swagger_auto_schema
# Create your views here.

@swagger_auto_schema(method='post', request_body=ArticleFindSerializer)
@api_view(['POST'])
...
def article_create(request):
    ...
    serializer = ArticleFindSerializer(data=data)
    if serializer.is_valid(raise_exception=True):
        serializer.save(user=request.user, city=city, animal=animal, species=species, borough=borough)
        return Response(serializer.data)

@swagger_auto_schema(method='put', request_body=ArticleFindSerializer)
@api_view(['GET', 'PUT'])
@permission_classes([IsAuthenticated])
def article_update(request, article_pk):
    ...
    if request.user == article.user:
        if request.method == 'GET':
            serializer = ArticleFindSerializer(article)
            return Response(serializer.data)
        else:
            data = {
            ...
            serializer = ArticleFindSerializer(article, data=data)
            city = get_object_or_404(City, city=request.data.get('city'))
            animal = get_object_or_404(Animal, kind=request.data.get('animal'))
            species = get_object_or_404(Species, species=request.data.get('species'), animal_id=animal.id)
            borough = get_object_or_404(Borough, gu=request.data.get('borough'))
...
def article_read(request, article_pk):
    ...
    """
    article = get_object_or_404(Article, pk=article_pk)
    serializer = ArticleFindDetailSerializer(instance=article)
    return Response(serializer.data)
...
@swagger_auto_schema(method='post', request_body=CommentFindSerializer)
@api_view(['POST'])
@permission_classes([IsAuthenticated])
def comment_create(request, article_pk):
    ...
    """
    article = get_object_or_404(Article, pk=article_pk)
    comment_serializer = CommentFindSerializer(data=request.data)
    if comment_serializer.is_valid(raise_exception=True):
        comment_serializer.save(user=request.user, article=article)
        article_serializer = ArticleFindDetailSerializer(article)
        return Response(article_serializer.data)

@swagger_auto_schema(method='put', request_body=CommentFindSerializer)
@api_view(['GET', 'PUT'])
@permission_classes([IsAuthenticated])
def comment_update(request, article_pk, comment_pk):
    ...
    if request.user == comment.user:
        if request.method == 'GET':
            serializer = CommentFindSerializer(comment)
            return Response(serializer.data)
        else:
            data = {
                'content': request.data.get('content', comment.content)
            }
            serializer = CommentFindSerializer(comment, data=data)
            if serializer.is_valid(raise_exception=True):
                serializer.save()
                return Response(serializer.data)
    else:
...
def article_list(request):
    ...
    serialzier = ArticleFindListSerializer(instance=articles, many=True)
    return Response(serialzier.data)

@swagger_auto_schema(method='POST', request_body=ArticleFindSearchSerializer)
@api_view(['POST'])
# @permission_classes([IsAuthenticated])
def article_search(request):
    ...
    serializer = ArticleFindListSerializer(instance=articles, many=True)
    return Response(serializer.data)