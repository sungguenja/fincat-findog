<template>
  <div>
    ...
    <b-button v-if="notice.user.username == user.username" class="mybutton1" @click="moveNoticeModify()">수정</b-button>
    <b-button  v-if="notice.user.username == user.username" class="mybutton2 mx-3" @click="moveNoticeDelete()">삭제</b-button>
...
export default {
  ...
  created() {
    ...
    axios({
      method: "GET",
      url: `${API_URL}/articles_find/` + this.$route.params.pk + "/",
  ...
  methods: {
    ...
    moveNoticeModify() {
      this.$router.push({
        path: "/articles_find/update/" + this.$route.params.pk,
      });
    },
    moveNoticeDelete() {
      const token = this.$cookies.get("auth-token");
      ...
      axios({
        method: "DELETE",
        url: `${API_URL}/articles_find/delete/` + this.$route.params.pk,
        headers: {
          Authorization: `Token ${token}`,
        },
      })
