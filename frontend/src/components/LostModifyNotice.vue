<template>
  <div>
    ...
    <div class="form-group">
      ...
      <input type="text" class="form-control my-2" placeholder="실종 일시" v-model="notice.time" />
    ...
    <b-button block class="mb-4" @click="moveLostNoticeDetail()">취소</b-button>
...
export default {
  ...
  created() {
    ...
    axios({
      method: "GET",
      url: `${API_URL}/articles_lost/update/` + this.$route.params.pk,
  ...
  methods: {
    submit() {
      ...
      axios({
        method: "PUT",
        url: `${API_URL}/articles_lost/update/` + this.find_pk + "/",
        data: {
        ...
        .then(({ data }) => {
          ...
          console.log(data.id);
          // if (data == 'success'){
          this.$router.push({ path: `/articles_lost/${this.find_pk}` });
          // }
        })
        .catch((err) => {
          alert(
    ...
    moveLostNoticeDetail() {
      this.$router.push({ path: `/articles_lost/${this.pk}` });
    },
